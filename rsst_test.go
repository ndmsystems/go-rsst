...
func TestResp10000(t *testing.T) {
	var r1000 = []byte{
		0x20, 0x00, 0, 1,
		0x20, 0x11, 0, 2,
		0x40, 0x04, 0x4b, 0x65, 0x65, 0x6e, 0x65, 0x74, 0x69, 0x63, 0x20, 0x47, 0x69, 0x67, 0x61, 0x00,
		0x10, 0x00,
		0x51, 0x00, 0x33, 0x2e, 0x30, 0x37, 0x2e, 0x41, 0x2e, 0x38, 0x2e, 0x30, 0x2d, 0x32, 0x00,
	}

	infos := UnpackResponse(r1000)
	require.True(t, len(infos) == 5)

	buf := PackResponse(infos)
	require.True(t, bytes.Equal(r1000, buf))
}
